...
{ // weight1, weight2, angle, delay (16 bits each) for AoA processing per Peak
	...
	firstPath = determine_tdde_firstpath(pktinfo, packetparams, tdde_ptr,
					     fftBuffer, MIN_comb);
...
{ // weight1, weight2, angle, delay (16 bits each) for AoA processing per Peak
	...
	if ((pktinfo->packetType == 0) && (pktinfo->sigBw > 0)) { // all legacy
								  // except full
								  // interpolation
								  // for 20in20
...
#endif
	{
		...
		interpolatePilotsParallel(pktinfo, fftInBuffer, ifftSizeOsf,
					  phaseRollNg, totalpower);
#else
	interpolatePilots(pktinfo, fftInBuffer, ifftSizeOsf, phaseRollNg,
			  totalpower);
#endif
	}
...
void wls_unpack_csi(unsigned int *bufferMemory, // CSI buffer
		    unsigned int *outBuffer, // 2k times (MAX_RX*MAX_TX+1)
					     // memory to output unpacked CSI
		    hal_wls_packet_params_t *packetparams, // values from
							   // Rx/Tx-Info, used
							   // mostly to find
							   // correct CSI buffer
		    hal_wls_processing_input_params_t *inputVals, // WLS / AoA
								  ...
								  // Parameters
								  // Structure
		    unsigned int *totalpower) // [MAX_RX*MAX_TX + 1] size to
					      // output power per Rx/Tx stream
{
	int csiDataSize;
	...
	if (pktinfo->packetType > 2) { // VHT+HE (Ng=1)
		...
		readHexDataDemodulateProcessVhtHeNg1Parallel(
			pktinfo, inputVals, bufferMemory + start_csi,
			csiDataSize, outBuffer, powerPerSubband, phaseRollNg);
#else
		readHexDataDemodulateProcessVhtHeNg1(
			pktinfo, inputVals, bufferMemory + start_csi,
			csiDataSize, outBuffer, powerPerSubband, phaseRollNg);
#endif
		if (pktinfo->sigBw == 3) // 160 MHz only
			detectPhaseJump(pktinfo, inputVals, outBuffer,
					phaseRollNg);
	} else { // Ng=2/4 (HT only)
#ifdef FFT_PARALLEL
		readHexDataDemodulateProcessParallel(
	...
	if ((pktinfo->packetType == 0) && (pktinfo->sigBw > 0)) { // all legacy
								  ...
								  // interpolation
								  // for 20in20
		processLegacyPackets(pktinfo, outBuffer, ifftSizeOsf,
				     phaseRollNg);
	}
#endif
#ifndef LEG_LTF_NG1
	...
	{
#if defined(FFT_PARALLEL) && defined(ARM_DS5)
		interpolatePilotsIntrinsic(pktinfo, outBuffer, ifftSizeOsf,
					   phaseRollNg, totalpower);
#elif defined(FFT_PARALLEL) && !defined(ARM_DS5)
		interpolatePilotsParallel(pktinfo, outBuffer, ifftSizeOsf,
					  phaseRollNg, totalpower);
#else
	interpolatePilots(pktinfo, outBuffer, ifftSizeOsf, phaseRollNg,
			  totalpower);
#endif
	}
#if defined(DEBUG_OUT_FD) && !defined(ARM_DS5)
...
int wls_calculate_toa(unsigned int *headerBuffer, // CSI BF_INFO header
		      unsigned int *fftInBuffer, // Input buffer (freq. domain
						 // CSI)
		      unsigned int *fftBuffer, // 2k times (MAX_RX*MAX_TX+1)
					       // scratch memory for time domain
					       // CSI
		      unsigned int *totalpower, // size [MAX_RX*MAX_TX + 1]
						// power per Rx/Tx stream
		      hal_wls_packet_params_t *packetparams, // values from
							     // Rx/Tx-Info, used
							     // mostly to find
							     // correct CSI
...
)
{
...
#ifdef ENABLE_SUBSPACE_FTIMING
	{
		...
		resVal = ((fineTimingRes << (14 - TOA_FPATH_BIPT)) / 5) >>
			 (pktinfo->fftSize + pktinfo->NgDsfShift);
	}
#else
	// final format is 32.TOA_FPATH_BIPT in micro seconds
	resVal = ((firstPathDelay << (14 - TOA_FPATH_BIPT)) / 5) >>
		 (pktinfo->fftSize + pktinfo->NgDsfShift);
#endif
#if defined(DEBUG_CYCLE_TIME)
	DEBUG_DELTA_TIME_US2 =
...
void wls_intialize_reference(unsigned int *headerBuffer, // CSI BF_INFO header
			     unsigned int *fftInBuffer, // Input buffer (freq.
							// domain CSI)
			     float *fftRefBuffer // Refernce CSI buffer (freq.
						 // domain CSI)
)
{
	int fftSize, ifftSizeOsf;
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			...
			writePtr = (float *)fftRefBuffer +
				   2 * ifftSizeOsf * (jj + ii * nRx);
...
void wls_update_reference_iir(hal_pktinfo_t *pktinfo,
			      float *fftRefBuffer, // Refernce CSI buffer (freq.
						   // domain CSI)
			      float *fftOutBuffer, // Output buffer float (freq.
						   // domain CSI)
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			...
			for (kk = 0; kk < ifftSizeOsf; kk++) {
				...
				H_old[2 * kk] = (1 - alpha) * tempRefI +
						alpha * tempValI;
				H_old[2 * kk + 1] = (1 - alpha) * tempRefQ +
						    alpha * tempValQ;
			}
		}
	}
...
void wls_update_reference_kalmman(hal_pktinfo_t *pktinfo,
				  float *fftRefBuffer, // Refernce CSI buffer
						       // (freq. domain CSI)
				  float *fftOutBuffer, // Output buffer float
						       // (freq. domain CSI)
				  UINT64 current_tsf)
{
	int ii, jj, kk;
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			...
			for (kk = 0; kk < ifftSizeOsf; kk++) {
				...
				H_old[2 * kk] =
					(1 - K_n) * tempRefI + K_n * tempValI;
				H_old[2 * kk + 1] =
					(1 - K_n) * tempRefQ + K_n * tempValQ;
			}
		}
	}
}
...
float wls_calc_rel_timing(hal_pktinfo_t *pktinfo,
			  ...
			  float *fftRefBuffer, // Refernce CSI buffer (freq.
					       // domain CSI)
			  unsigned int *tempBuffer, // 2k times
						    // (MAX_RX*MAX_TX+1) scratch
						    // memory
			  float *psiArray // [MAX_RX*MAX_TX] float array of
					  // phase on each path
)
{
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			...
			for (kk = 0; kk < ifftSizeOsf; kk++) {
				...
				tempI = tempValI * tempRefI +
					tempValQ * tempRefQ;
				tempQ = tempValQ * tempRefI -
					tempValI * tempRefQ;
			...
			// IFFT
			radix2IfftFlt(procBuffer, ifftSizeOsf, twiddleTableFlt,
				      MAX_FFT_FLT);
			for (kk = 0; kk < ifftSizeOsf; kk++) {
				double sumVal = (double)procBuffer[2 * kk] *
						procBuffer[2 * kk];
				sumVal += (double)procBuffer[2 * kk + 1] *
					  procBuffer[2 * kk + 1];
...
void wls_remove_phase(hal_pktinfo_t *pktinfo,
		      ...
		      float *psiArray, // [MAX_RX*MAX_TX] float array of phase
				       // on each path
		      unsigned int *fftInBuffer, // Input buffer (freq. domain
						 // CSI)
		      float *fftOutBuffer // Output buffer float (freq. domain
					  // CSI)
)
{
	int ii, jj, kk;
...
float wls_calc_perturbation(hal_pktinfo_t *pktinfo,
			    float *fftRefBuffer, // H_old - refernce CSI buffer
						 // (freq. domain / float)
			    float *fftOutBuffer // H_comp - compensated CSI
						// (freq. domain / float)
)
{
	int ii, jj, kk;
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			...
			for (kk = 0; kk < ifftSizeOsf; kk++) {
				...
				sumI += tempValI * tempRefI +
					tempValQ * tempRefQ;
				sumQ += tempValQ * tempRefI -
					tempValI * tempRefQ;

				norm_comp += tempValI * tempValI +
					     tempValQ * tempValQ;
				norm_ref += tempRefI * tempRefI +
					    tempRefQ * tempRefQ;
			}
	...
	// InitHcorr0(kk) = sum(sum(abs(H_comp). ^ 2) - 2 * abs(H_proj). ^ 2 +
	// sum(abs(H_old). ^ 2).*abs(H_proj). ^ 2) / (N_t*N_r); InitHcorr0(kk) =
	// sum(1 - abs(H_proj). ^ 2) / (N_t*N_r);
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			float H_proj_I = H_proj[2 * (jj + ii * nRx)];
			float H_proj_Q = H_proj[2 * (jj + ii * nRx) + 1];
			float abs_H_proj =
				H_proj_I * H_proj_I + H_proj_Q * H_proj_Q;
			float norm_comp = H_comp_norm[jj + ii * nRx];
			float norm_ref = H_ref_norm[jj + ii * nRx];
			float tempVal =
				1.0f - abs_H_proj / (norm_comp * norm_ref);
...
float wls_update_cross_corr_pi_calc(unsigned int *headerBuffer, // CSI BF_INFO
								// header
				    unsigned int *fftInBuffer, // Input buffer
							       // (freq. domain
							       // CSI)
				    float *fftRefBuffer, // Refernce CSI buffer
							 // (freq. domain CSI)
...
)
{
	...
	phi_hat = wls_calc_rel_timing(pktinfo, fftInBuffer, fftRefBuffer,
				      tempBuffer, psiArray);

	wls_remove_phase(pktinfo, phi_hat, psiArray, fftInBuffer,
			 (float *)tempBuffer);

	perturbVal = wls_calc_perturbation(pktinfo, fftRefBuffer,
					   (float *)tempBuffer);
#if defined(PI_ALPHA_FACTOR)
	wls_update_reference_iir(pktinfo, fftRefBuffer, (float *)tempBuffer,
				 PI_ALPHA_FACTOR);
...
#endif
		wls_update_reference_kalmman(pktinfo, fftRefBuffer,
					     (float *)tempBuffer, TSF);
	}
#endif
	return perturbVal;
}
