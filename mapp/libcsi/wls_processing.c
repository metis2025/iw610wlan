...
const unsigned char pilotToneIndexLeg80[SC5_LEG80_PILOTS] = {
	11,  25,  32,  39,  53,	 75,  89,  96,	103, 117,
	139, 153, 160, 167, 181, 203, 217, 224, 231, 245};
const unsigned char pilotToneIndexVHT40[SC5_VHT40_PILOTS] = {11, 39, 53,
							     75, 89, 117};
const unsigned char pilotToneIndexLeg40[SC5_LEG40_PILOTS] = {
	11, 25, 32, 39, 53, 75, 89, 96, 103, 117};
...
const short phiCorr64[64] = {
	...
	P(2049)};
const short phiCorrScBt64[64] = {
	P(-3584), P(-2286), P(-1886), P(-1642), P(-1466), P(-1328), P(-1213),
	P(-1114), P(-1027), P(-950),  P(-881),	P(-817),  P(-759),  P(-705),
	P(-655),  P(-607),  P(-562),  P(-519),	P(-478),  P(-439),  P(-401),
	P(-364),  P(-328),  P(-293),  P(-259),	P(-225),  P(-192),  P(-159),
	P(-127),  P(-95),   P(-63),   P(-32),	P(0),	  P(32),    P(63),
	P(95),	  P(127),   P(159),   P(192),	P(225),	  P(259),   P(293),
	P(328),	  P(364),   P(401),   P(439),	P(478),	  P(519),   P(562),
	P(607),	  P(655),   P(705),   P(759),	P(817),	  P(881),   P(950),
	P(1027),  P(1114),  P(1213),  P(1328),	P(1466),  P(1642),  P(1886),
	P(2286)};
#else
...
void removeIirPhaseTdde(short *A0_vals_ptr, int fft_size,
			const short *phiCorrPtr, int phiCorrPtrStride)
{
	...
	for (ii = 0; ii < fft_size; ii++) {
		...
		idx = phiCorrPtr[phiCorrIdx];
		phiCorrIdx += phiCorrPtrStride;
...
int findfirstpath(hal_pktinfo_t *pktinfo, short *A0_ptr,
		  unsigned int *tdde_power, int fft_size, char MIN_comb,
		  unsigned short path_idx)
{
	...
	if (MIN_comb || (path_idx == 0)) {
		for (kk = 0; kk < fft_size; kk++) {
			tdde_power[kk] =
				A0_ptr[2 * kk] * A0_ptr[2 * kk] +
				A0_ptr[2 * kk + 1] * A0_ptr[2 * kk + 1];
		}
	} else {
		for (kk = 0; kk < fft_size; kk++) {
			tdde_power[kk] +=
	...
	for (ii = 1; ii <= maxIdx; ii++) {
		if ((tdde_power[ii] > detThresh) &
		    (tdde_power[ii] >= tdde_power[ii + 1])) {
			firstPathIdx = ii;
			break;
		}
	}
	if (firstPathIdx == 0) {
		...
		if (ii > TOA_FPATH_BIPT) {
			...
			tempVal <<= (ii - 1);
			delta = tempVal /
				(denominator >> (TOA_FPATH_BIPT - ii + 1));
		}
...
int determine_tdde_firstpath(hal_pktinfo_t *pktinfo,
			     ...
			     unsigned int *procBuffer, char MIN_comb)
{
	int ii, jj, kk;
	int num_IQ_samps, fft_size, freq_size, ifft_size, tdde_offset,
		firstPathIdx, TDDEFirstPath;
	short M_idx;
	int ret_firstPath[2];
	short data_offset[2];
	...
	A0_vals_intepr =
		(unsigned int *)(procBuffer + 2 * ifft_size); // skip A0_vals
	...
	for (jj = 0; jj < numTx; jj++) {
		for (ii = 0; ii < numRx; ii++) {
			memset(A0_vals_bfr, 0,
			       ifft_size * sizeof(unsigned int));
			...
			// transfer to frequency domain for interpolation and
			// phase removal use IFFT instead of FFFT for code
			// simplicity, needs reversed input
			radix2Ifft(A0_vals_bfr, fft_size, radix4FftTwiddleArr,
				   MAX_FFT_SIZE);
			...
			removeIirPhaseTdde(A0_vals_bfr, freq_size, phiCorrPtr,
					   phiCorrStep);
#endif
			// use IFFT twice to interpolate, bit growth
			radix2Ifft(A0_vals_bfr, ifft_size, radix4FftTwiddleArr,
				   MAX_FFT_SIZE);

			firstPathIdx = findfirstpath(pktinfo, A0_vals_bfr,
						     A0_vals_intepr, ifft_size,
						     MIN_comb, ii);
		...
		ret_firstPath[jj] = tempvalue + ((data_offset[jj] - tdde_offset)
						 << (TOA_FPATH_BIPT + sigBw));
		tempvalue = TDDE_IDX_MAX;
	}
	if (numTx == 1)
		tempvalue = ret_firstPath[0];
	else if (MIN_comb) { // min across streams
		tempvalue = (ret_firstPath[0] < ret_firstPath[1]) ?
				    ret_firstPath[0] :
				    ret_firstPath[1];
	} else { // use average across streams
...
void readHexDataDemodulateProcessVhtHeNg1(
	...
	unsigned int *powerPerSubband, int *phaseRollPtr)
{
	...
	int phiCorrScale = 0;
	int channelOffset = 0; // Default for deviceBandwidth == bandwidth
	...
	if (bandwidth < deviceBandwidth) { // figure out channel offset
		if (deviceBandwidth == 80) {
			if (bandwidth == 20) {
				channelOffset = psb;
			} else if (bandwidth == 40) {
				channelOffset = (psb < 2) ? 0 : 1;
			}
		} else if ((deviceBandwidth == 40) && (bandwidth == 20)) {
			channelOffset = psb & 0x1;
		}
		// add offset
		phiCorrPtr +=
	...
	if ((pktinfo->packetType == 3) ||
	    (pktinfo->packetType == 0)) { // VHT + legacy
		pilotSkip = 0;
		dcSkip = 0;
	...
	for (ii = 0; ii < nTx; ii++) {
		for (jj = 0; jj < nRx; jj++) {
			for (bb = 1; bb < numSubbands; bb++) {
				...
				idx = (6 * phaseRollNg * MAX_FFT_SIZE) >>
				      MPY_BIPT;
				idx &= (MAX_FFT_SIZE - 1); // apply modulo
				tempValLd = cosTablePtr[idx];
				cosPhiRight = (tempValLd & 0xffff);
				sinPhiRight = (tempValLd >> 16);
				resMidI = (((int)cosPhiRight * tempRightI) +
					   ((int)sinPhiRight * tempRightQ)) /
					  2;
				resMidQ = (((int)cosPhiRight * tempRightQ) -
					   ((int)sinPhiRight * tempRightI)) /
					  2;

				cosPhiLeft = cosPhiRight;
				sinPhiLeft = sinPhiRight;
				resMidI += (((int)cosPhiLeft * tempLeftI) -
					    ((int)sinPhiLeft * tempLeftQ)) /
					   2;
				resMidQ += (((int)cosPhiLeft * tempLeftQ) +
					    ((int)sinPhiLeft * tempLeftI)) /
					   2;

				convertI = resMidI >> TWIDDLE_BIPT;
				convertQ = resMidQ >> TWIDDLE_BIPT;
				tempValSt =
					(convertI & 0xffff) | (convertQ << 16);
				*readPtr = tempValSt;

				for (kk = 0; kk < 5; kk++) {
					distRight = 5 - kk;
					distLeft = 7 + kk;
					idx = (distRight * phaseRollNg *
					       MAX_FFT_SIZE) >>
					      MPY_BIPT;
					idx &= (MAX_FFT_SIZE - 1); // apply
								   // modulo
					tempValLd = cosTablePtr[idx];
					cosPhiRight = (tempValLd & 0xffff);
					sinPhiRight = (tempValLd >> 16);

					resRightI = (((int)cosPhiRight *
						      tempRightI) +
						     ((int)sinPhiRight *
						      tempRightQ)) /
						    12 * distLeft;
					resRightQ = (((int)cosPhiRight *
						      tempRightQ) -
						     ((int)sinPhiRight *
						      tempRightI)) /
						    12 * distLeft;

					resLeftI = (((int)cosPhiRight *
						     tempLeftI) -
						    ((int)sinPhiRight *
						     tempLeftQ)) /
						   12 * distLeft;
					resLeftQ = (((int)cosPhiRight *
						     tempLeftQ) +
						    ((int)sinPhiRight *
						     tempLeftI)) /
						   12 * distLeft;

					idx = (distLeft * phaseRollNg *
					       MAX_FFT_SIZE) >>
					      MPY_BIPT;
					idx &= (MAX_FFT_SIZE - 1); // apply
								   // modulo
					...
					resRightI +=
						(((int)cosPhiLeft * tempLeftI) -
						 ((int)sinPhiLeft *
						  tempLeftQ)) /
						12 * distRight;
					resRightQ +=
						(((int)cosPhiLeft * tempLeftQ) +
						 ((int)sinPhiLeft *
						  tempLeftI)) /
						12 * distRight;

					resLeftI += (((int)cosPhiLeft *
						      tempRightI) +
						     ((int)sinPhiLeft *
						      tempRightQ)) /
						    12 * distRight;
					resLeftQ += (((int)cosPhiLeft *
						      tempRightQ) -
						     ((int)sinPhiLeft *
						      tempRightI)) /
						    12 * distRight;
	...
	if (pktinfo->packetType == 3) { // VHT
		...
		if (pktinfo->packetType == 0) {
			...
			if ((bandwidth == 80) || (bandwidth == 160)) {
				...
				// VHT pilots skipped [103, 75, 39, 11, -103,
				// -75, -39, -11] legacy pilots
				// [-117,-103,-89,-75,-53,-39,-25,-11,11,25,39,53,75,89,103,117]
				// some overlap extra DC copy neighbor
				// [-96,-32,32,96]
				...
				// legacy pilots [-53,-39,-25,-11,11,25,39,53]
				// some overlap extra DC copy neighbor [-32,32]
...
#endif
			{
				...
				NgShift = 1; // Ng=2
				extraSc = 0; // 2; // change phase on two
					     // subcarriers
				powerScale = 2;
			}
